 ...
 * @returns
 */
export async function resolve(didUri: any, options: any = { }) {
...
export async function anchor(anchorRequest: any, options: any = { }) {
  const {
  ...
  const pow: any = ProofOfWorkSDK;
  return pow.submitIonRequest(challengeEndpoint, solutionEndpoint, JSON.stringify(anchorRequest));
};