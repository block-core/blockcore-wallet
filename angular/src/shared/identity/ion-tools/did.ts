...
export class DID {
  ...
  async generateOperation(type: any, content: any, commit = true) {
  ...
  async #generateOperation(type: any, content: any, commit: any) {
    ...
    if (type !== 'create') {
      op.previous = this.#ops.reduce((last: any, op: any) => {
  ...
  async generateRequest(payload = 0, options: any = {}) {
    let op;
